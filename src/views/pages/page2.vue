...
export default {
  ...
  methods: {
    ...
    getList() {
      fetch(url + "/area/region="+ this.province +"&page=" + this.page, {
        method: "GET",
        mode: "cors"
      }).then(res => {
        return res.json();
    ...
    drawBar() {
      ...
      bar_option.xAxis[0].data = _.map(this.region_list, item => {
        return item.region;
      });
      bar_option.series[0].data = _.map(this.region_list, item => {
    ...
    drawPie() {
      ...
      pie_option.legend.data = _.map(this.region_list, item => {
        return item.region;
      });
      pie_option.series[0].data = _.map(this.region_list, item => {
        let obj = {
          name: item.region,
          value: item.region_count
        };
        return obj;
      });
